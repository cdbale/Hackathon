...
max_post_prob <- function(protected_data, ids, ei_tuples, disease_status, prior){
  ...
  # select just latitude and longitude for the tree
  # tree_data <- ds %>% select(latitude, longitude)
  
  # get indexes of the nearest neighbor to each point
  neighbor_indexes <- get.knnx(data=protected_data[,c('latitude', 'longitude')], query=ei_tuples[,c('latitude', 'longitude')], k=1, algorithm="kd_tree")$nn.index
...
Reduce to tidy data and plot.
```{r}
# colnames(full_ma_inf) <- ma_k
# colnames(s_ma_inf) <- ma_k
# 
# full_ma_inf <- full_ma_inf %>%
#   gather(key="k", value="Inf_condition")
# 
# s_ma_inf <- s_ma_inf %>%
#   gather(key="k", value="Inf_condition")
```
